...
public:
    FramebufferRenderer(const std::string& fbDevice = "/dev/fb0");
    ~FramebufferRenderer();
    
    // Window management (framebuffer initialization)
    ...
    void present(); // No-op for framebuffer (direct writes)
    ...
    // Drawing primitives - Text (bitmap font rendering)
    ...
    // Getters
    void* getWindow() const { return nullptr; } // No window concept for framebuffer
    void* getRenderer() const { return framebuffer_; }
    int getWidth() const { return width_; }
    int getHeight() const { return height_; }
    
    // Event handling (simplified - no-op for framebuffer mode)
    bool pollEvent(void* event) { return false; } // Stub for compatibility
    
    // Click/touch event handling (can be extended to read from /dev/input)
    ...
    void processEvents(VNode::Ptr root) {} // Stub for compatibility
...
private:
    ...
    // Internal bitmap font for text rendering
    void renderBitmapChar(int x, int y, char c, uint32_t color, int scale);
    
    bool initializeFramebuffer();
    void cleanup();
    void setPixel(int x, int y, uint32_t color);
    uint32_t getPixel(int x, int y);
    uint32_t convertColorToFramebufferFormat(uint32_t color);
